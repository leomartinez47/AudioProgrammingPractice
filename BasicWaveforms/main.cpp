...
int main() {
  ...
  Waveform triangleWave("triangle", 15000, 440, 44100, 1); // doesnt work the sample value is only ever 4
  // sine works
  //Waveform sineWave("sine", 15000, 440, 44100, 1); 
  int dataSize = triangleWave.getTotalSamples();
  //just write data so we can check shapes
  for(int i = 0; i < dataSize; i++) {
    cout << triangleWave << endl;
    triangleWave++;
  }
}